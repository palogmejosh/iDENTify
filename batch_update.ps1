# Batch update script for fixed navigation
$files = @(
    'cod_patients.php',
    'settings.php', 
    'profile.php',
...
foreach ($file in $files) {
    ...
    if ($content -match '(?s)<body[^>]*>\s*(?:<!--[^>]*-->\s*)?<header class="bg-gradient-to-r.*?</header>\s*<div class="flex">\s*<!--\s*Sidebar\s*-->\s*<nav class="w-64.*?</nav>\s*<!--\s*Main Content\s*-->\s*<main class="flex-1') {
        $content = $content -replace '(?s)(<body[^>]*>)\s*(<!--[^>]*-->)?\s*<header class="bg-gradient-to-r.*?</header>\s*<div class="flex">\s*<!--\s*Sidebar\s*-->\s*<nav class="w-64.*?</nav>\s*<!--\s*Main Content\s*-->\s*<main class="flex-1', '$1
